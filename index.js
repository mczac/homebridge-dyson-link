...
class DysonPlatform {
      ...
      this.api.on('didFinishLaunching', () => {
        ...
        if (this.config && this.config.accessories) {
          ...
          this.getDevicesFromAccount(accountEmail, accountPassword, config.country, (accountDevices) => {
            ...
            this.deleteUnusedAccessories(accountDevices) // Serail number isn't unified
            // this.config.accessories - accessories in CONFIG
            // accountDevices[accessory.serialNumber] - acceories from the CLOUD account
            ...
            this.config.accessories.forEach((accessory) => {
              ...
              let monitoringModeVisible = accessory.monitoringModeVisible;
              if (monitoringModeVisible == null || monitoringModeVisible == undefined) {
                platform.log.debug("no monitoring mode visible value, default to true");
                monitoringModeVisible = true;
              }
              ...
              if (device.valid) {
                ...
                if (!cachedAccessory) {
                  ...
                  new DysonLinkAccessory(accessory.displayName, device, dysonAccessory, platform.log, nightModeVisible, focusModeVisible, autoModeVisible, monitoringModeVisible);
                  ...
                  new DysonLinkAccessory(accessory.displayName, device, cachedAccessory, platform.log, nightModeVisible, focusModeVisible, autoModeVisible, monitoringModeVisible);
  ...
  deleteUnusedAccessories(accountDevices) {


    const accessoryNotExist = (cachedAccessory) => {

      return this.config.accessories.find((accessory) => {

          let serialNumber = accessory.serialNumber || ''
          let deviceInfo = accountDevices[serialNumber]

          if (deviceInfo) serialNumber = 'DYSON-' + accessory.serialNumber + '-' + deviceInfo.ProductType;

          return ( UUIDGen.generate(serialNumber) === cachedAccessory.UUID )
      }) === undefined
  }


  for (let i = this.accessories.length - 1; i >= 0; i -= 1) {
    if (accessoryNotExist(this.accessories[i])) {
      // Cached item doesn't exist in the config anymore, deleting
      this.log(`Removing unused accessory: ${this.accessories[i].displayName} with ${this.accessories[i].UUID}`)
      this.api.unregisterPlatformAccessories("homebridge-dyson-link", "DysonPlatform", [this.accessories[i]])
